...
/obj/structure/vampdoor/examine(mob/user)
	...
	var/difference = (H.get_stat(STAT_LARCENY) * 2 + H.get_stat(STAT_DEXTERITY)) - lockpick_difficulty //Lower number = higher difficulty
	...
	if(istype(W, /obj/item/vamp/keys/hack))
		...
		if(locked)
			...
			for(var/mob/living/carbon/human/npc/police/P in oviewers(7, src))
				if(P)
					P.Aggro(user)
			var/total_lockpicking = user.get_stat(STAT_LARCENY)
			if(do_mob(user, src, (lockpick_timer - total_lockpicking * 2) SECONDS))
				var/roll = rand(1, 20) + (total_lockpicking * 2 + user.get_stat(STAT_DEXTERITY)) - lockpick_difficulty
				if(roll <=1)
