...
/mob/living/carbon/human/proc/SwitchBlocking()
	if(!blocking)
		...
		var/mutable_appearance/block_overlay = mutable_appearance('modular_darkpack/icons/icons.dmi', "block", -FIGHT_LAYER)
...
/mob/living/carbon/human/attackby(obj/item/W, mob/living/user, params)
	...
	if(user == parrying && user != src)
		...
		playsound(src, 'modular_darkpack/sound/parried.ogg', 70, TRUE)
		clear_parrying()
		return
...
/mob/living/carbon/human/proc/Parry(mob/M)
	if(!pulledby && !parrying && world.time-parry_cd >= 30 && M != src)
		...
		playsound(src, 'modular_darkpack/sound/parry.ogg', 70, TRUE)
		remove_overlay(FIGHT_LAYER)
		var/mutable_appearance/parry_overlay = mutable_appearance('modular_darkpack/icons/icons.dmi', "parry", -FIGHT_LAYER)
