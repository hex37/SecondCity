...
/obj/effect/vip_barrier/proc/handle_social_bypass(mob/living/carbon/human/user, mob/bouncer, used_badge = FALSE)
	...
	if(!do_mob(user, bouncer, max(5 SECONDS, social_bypass_time - (user.get_stat(STAT_CHARISMA) * 2 SECONDS))))
		return
	...
	if(SSroll.storyteller_roll(user.get_stat(STAT_CHARISMA), involved_social_roll, mobs_to_show_output = user) == ROLL_SUCCESS)
