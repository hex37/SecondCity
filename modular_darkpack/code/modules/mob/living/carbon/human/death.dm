/mob/living/carbon/human/death()
	...
	if(iskindred(src) || iscathayan(src))
		...
		if(in_frenzy)
			exit_frenzymod()
		SEND_SOUND(src, sound('modular_darkpack/sound/final_death.ogg', 0, 0, 50))
		...
		switch (years_undead)
			...
			if (200 to INFINITY) //turn to ash
				if (iskindred(src))
					playsound(src, 'modular_darkpack/sound/burning_death.ogg', 80, TRUE)
				else if (iscathayan(src))
					playsound(src, 'modular_darkpack/sound/vicissitude.ogg', 80, TRUE)
				lying_fix()
				dir = SOUTH
