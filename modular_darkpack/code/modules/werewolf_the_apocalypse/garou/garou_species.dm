...
/proc/adjust_rage(amount, mob/living/carbon/C, sound = TRUE)
	if(amount > 0)
		if(C.auspice.rage < 10)
			if(sound)
				SEND_SOUND(C, sound('modular_darkpack/sound/rage_increase.ogg', 0, 0, 75))
	...
	if(amount < 0)
		if(C.auspice.rage > 0)
			...
			if(sound)
				SEND_SOUND(C, sound('modular_darkpack/sound/rage_decrease.ogg', 0, 0, 75))
...
/proc/adjust_gnosis(amount, mob/living/carbon/C, sound = TRUE)
	if(amount > 0)
		if(C.auspice.gnosis < C.auspice.start_gnosis)
			if(sound)
				SEND_SOUND(C, sound('modular_darkpack/sound/humanity_gain.ogg', 0, 0, 75))
