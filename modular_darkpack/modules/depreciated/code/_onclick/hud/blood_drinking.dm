	...
	if(ishuman(usr))
		...
		if(BD.grab_state > GRAB_PASSIVE)
			if(ishuman(BD.pulling))
				...
				if(isghoul(BD))
					if(!iskindred(PB))
						SEND_SOUND(BD, sound('modular_darkpack/modules/depreciated/sound/need_blood.ogg', 0, 0, 75))
			...
			if(isliving(BD.pulling))
				...
				if(!skipface)
					if(!HAS_TRAIT(BD, TRAIT_BLOODY_LOVER))
						playsound(BD, 'modular_darkpack/modules/depreciated/sound/drinkblood1.ogg', 50, TRUE)
					...
					if(!HAS_TRAIT(BD, TRAIT_BLOODY_LOVER))
						...
						playsound(BD, 'modular_darkpack/modules/depreciated/sound/kiss.ogg', 50, TRUE)
