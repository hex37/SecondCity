					...
					if(C.stat != DEAD)
						if(C.auspice.tribe == tribe)
							if(last_rage+50 < world.time)
								...
								SEND_SOUND(C, sound('modular_darkpack/sound/bumps.ogg', 0, 0, 75))
					...
					if(iswerewolf(C) || isgarou(C))
						if(C.stat != DEAD)
							if(C.auspice.tribe == tribe)
								...
								SEND_SOUND(C, sound('modular_darkpack/sound/inspire.ogg', 0, 0, 75))
...
/obj/structure/werewolf_totem/spiral
	name = "Spiral Totem"
	desc = "Gives power to all Garou of that tribe and steals it from others."
	icon = 'modular_darkpack/icons/64x32.dmi'
	icon_state = "spiral"
				...
				if(C.auspice.name == "Theurge")
					if(!opening)
						...
						if(do_mob(user, src, 10 SECONDS))
							playsound(loc, 'modular_darkpack/sound/portal.ogg', 75, FALSE)
