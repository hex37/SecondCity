/atom/movable/screen/drinkblood
	name = "Drink Blood"
	icon = 'modular_darkpack/icons/disciplines.dmi'
	layer = HUD_LAYER
	plane = HUD_PLANE
	...
	if(ishuman(usr))
		...
		if(BD.grab_state > GRAB_PASSIVE)
			if(ishuman(BD.pulling))
				...
				if(isghoul(BD))
					if(!iskindred(PB))
						SEND_SOUND(BD, sound('modular_darkpack/sound/need_blood.ogg', 0, 0, 75))
			...
			if(isliving(BD.pulling))
				...
				if(!skipface)
					if(!HAS_TRAIT(BD, TRAIT_BLOODY_LOVER))
						playsound(BD, 'modular_darkpack/sound/drinkblood1.ogg', 50, TRUE)
					...
					if(!HAS_TRAIT(BD, TRAIT_BLOODY_LOVER))
						...
						playsound(BD, 'modular_darkpack/sound/kiss.ogg', 50, TRUE)
