...
/obj/item/vamp/phone
	...
	var/choosed_number = ""
	var/last_call = 0
	var/call_sound = 'modular_darkpack/sound/call.ogg'
	var/can_fold = 1
...
//	var/mob/living/carbon/human/V = usr
	...
	switch(action)
		if("hang")
			...
			if(online)
				if(!silence)
					playsound(online, 'modular_darkpack/sound/phonestop.ogg', 25, FALSE)
				online.online = null
				online = null
			.= TRUE
		if("accept")
	...
	if(last_call+100 <= world.time && !talking)
		...
		if(online)
			if(online.silence == FALSE)
				playsound(src, 'modular_darkpack/sound/phonestop.ogg', 25, FALSE)
			online.online = null
			online = null
	if(!talking && online)
		if(online.silence == FALSE)
			playsound(src, 'modular_darkpack/sound/phone.ogg', 10, FALSE)
			...
			if(isliving(hearing_args[HEARING_SPEAKER]))
				...
				if(ishuman(hearing_args[HEARING_SPEAKER]))
					...
					if (HAS_TRAIT(SPK, TRAIT_REJECTED_BY_TECHNOLOGY))
						...
						playsound(online, 'modular_darkpack/sound/lasombra_whisper.ogg', 50, FALSE)
					else
						playsound(online, 'modular_darkpack/sound/phonetalk.ogg', 50, FALSE)
...
/obj/item/vamp/phone/street
	...
	onflooricon = 'modular_darkpack/icons/props.dmi'
	icon_state = "payphone"
	anchored = TRUE
	number = "1447"
	can_fold = 0
...
/obj/item/vamp/phone/clean
	...
	icon = 'modular_darkpack/icons/onfloor.dmi'
	icon_state = "redphone"
	anchored = TRUE
	number = "700 4424"
	can_fold = 0
