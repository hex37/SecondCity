/datum/action/gift
	icon_icon = 'modular_darkpack/icons/werewolf_abilities.dmi'
	button_icon = 'modular_darkpack/icons/werewolf_abilities.dmi'
	...
	if(istype(owner, /mob/living/carbon))
		...
		if(rage_req)
			if(H.auspice.rage < rage_req)
				...
				SEND_SOUND(owner, sound('modular_darkpack/sound/werewolf_cast_failed.ogg', 0, 0, 75))
				allowed_to_proceed = FALSE
				return
...
/datum/action/gift/falling_touch/Trigger()
	...
	if(allowed_to_proceed)
		var/mob/living/carbon/H = owner
		playsound(get_turf(owner), 'modular_darkpack/sound/falling_touch.ogg', 75, FALSE)
...
/datum/action/gift/inspiration/Trigger()
	...
	if(allowed_to_proceed)
		var/mob/living/carbon/H = owner
		playsound(get_turf(owner), 'modular_darkpack/sound/inspiration.ogg', 75, FALSE)
		H.emote("scream")
...
/datum/action/gift/razor_claws/Trigger()
	...
	if(allowed_to_proceed)
		if(ishuman(owner))
			playsound(get_turf(owner), 'modular_darkpack/sound/razor_claws.ogg', 75, FALSE)
...
/datum/action/gift/beast_speech/Trigger()
	...
	if(allowed_to_proceed)
		...
		if(length(C.beastmaster) > 3)
			...
			qdel(B)
		playsound(get_turf(owner), 'modular_darkpack/sound/wolves.ogg', 75, FALSE)
...
/datum/action/gift/call_of_the_wyld/Trigger()
	...
	if(allowed_to_proceed)
		var/mob/living/carbon/C = owner
		C.emote("howl")
		playsound(get_turf(C), pick('modular_darkpack/sound/awo1.ogg', 'modular_darkpack/sound/awo2.ogg'), 100, FALSE)
		for(var/mob/living/carbon/A in orange(6, owner))
			if(A)
				if(isgarou(A) || iswerewolf(A))
					A.emote("howl")
					playsound(get_turf(A), pick('modular_darkpack/sound/awo1.ogg', 'modular_darkpack/sound/awo2.ogg'), 100, FALSE)
					spawn(10)
...
/datum/action/gift/resist_pain/Trigger()
	...
	if(allowed_to_proceed)
		if(ishuman(owner))
			playsound(get_turf(owner), 'modular_darkpack/sound/resist_pain.ogg', 75, FALSE)
			var/mob/living/carbon/human/H = owner
...
/datum/action/gift/sense_wyrm/Trigger()
	...
	if(allowed_to_proceed)
		...
		playsound(get_turf(owner), 'modular_darkpack/sound/sense_wyrm.ogg', 75, FALSE)
...
/datum/action/gift/blur_of_the_milky_eye/Trigger()
	...
	if(allowed_to_proceed)
		var/mob/living/carbon/C = owner
		C.alpha = 36
		playsound(get_turf(owner), 'modular_darkpack/sound/milky_blur.ogg', 75, FALSE)
		spawn(20 SECONDS)
			C.alpha = 255
...
/datum/action/gift/infectious_laughter/Trigger()
	...
	if(allowed_to_proceed)
		...
		C.emote("laugh")
		C.Stun(10)
		playsound(get_turf(owner), 'modular_darkpack/sound/infectious_laughter.ogg', 100, FALSE)
...
/datum/action/gift/rage_heal/Trigger()
	...
	if(allowed_to_proceed)
		var/mob/living/carbon/C = owner
		if(C.stat != DEAD)
			SEND_SOUND(owner, sound('modular_darkpack/sound/rage_heal.ogg', 0, 0, 75))
...
/datum/action/gift/hispo/Trigger()
	...
	if(allowed_to_proceed)
		...
		playsound(get_turf(owner), 'modular_darkpack/sound/transform.ogg', 50, FALSE)
		if(H.hispo)
			H.icon = 'modular_darkpack/icons/werewolf_lupus.dmi'
			H.pixel_w = 0
			H.pixel_z = 0
			H.melee_damage_lower = initial(H.melee_damage_lower)
			H.melee_damage_upper = initial(H.melee_damage_upper)
			...
			H.icon = 'modular_darkpack/icons/hispo.dmi'
			H.pixel_w = -16
			H.pixel_z = -16
...
/datum/action/gift/glabro/Trigger()
	...
	if(allowed_to_proceed)
		...
		if(G.glabro)
			...
			H.remove_overlay(PROTEAN_LAYER)
			var/mutable_appearance/glabro_overlay = mutable_appearance('modular_darkpack/icons/werewolf_abilities.dmi', H.transformator.crinos_form?.sprite_color, -PROTEAN_LAYER)
