/obj/structure/vampdoor
	...
	icon = 'modular_darkpack/icons/doors.dmi'
	icon_state = "door-1"
	plane = GAME_PLANE
	...
	var/open_sound = 'modular_darkpack/sound/door_open.ogg'
	var/close_sound = 'modular_darkpack/sound/door_close.ogg'
	var/lock_sound = 'modular_darkpack/sound/door_locked.ogg'
	var/burnable = FALSE
			...
			if(ishuman(user))
				...
				if(H.potential > 0)
					if((H.potential * 2) >= lockpick_difficulty)
						playsound(get_turf(src), 'modular_darkpack/sound/get_bent.ogg', 100, FALSE)
						...
						playsound(get_turf(src), 'modular_darkpack/sound/get_bent.ogg', 50, TRUE)
						proc_unlock(5)
					...
					pixel_z = pixel_z+rand(-1, 1)
					pixel_w = pixel_w+rand(-1, 1)
					playsound(src, 'modular_darkpack/sound/knock.ogg', 75, TRUE)
					to_chat(user, "<span class='warning'>[src] is locked!</span>")
					spawn(2)
						pixel_z = initial(pixel_z)
						pixel_w = initial(pixel_w)
...
/obj/structure/vampdoor/attackby(obj/item/W, mob/living/user, params)
	...
	if(istype(W, /obj/item/vamp/keys/hack))
		...
		if(locked)
			hacking = TRUE
			proc_unlock(5)
			playsound(src, 'modular_darkpack/sound/hack.ogg', 100, TRUE)
