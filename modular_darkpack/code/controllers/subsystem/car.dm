...
/obj/item/gas_can
	...
	lefthand_file = 'modular_darkpack/icons/righthand.dmi'
	righthand_file = 'modular_darkpack/icons/lefthand.dmi'
...
/obj/item/gas_can/afterattack(atom/A, mob/user, proximity)
	...
	if(istype(get_turf(A), /turf/open/floor) && !istype(A, /obj/vampire_car) && !istype(A, /obj/structure/fuelstation) && !istype(A, /mob/living/carbon/human) && !istype(A, /obj/structure/drill))
		...
		playsound(get_turf(src), 'modular_darkpack/sound/gas_splat.ogg', 50, TRUE)
	if(istype(A, /mob/living/carbon/human))
		...
		playsound(get_turf(H), 'modular_darkpack/sound/gas_splat.ogg', 50, TRUE)
...
/obj/vampire_car/attack_hand(mob/user)
	...
	if(ishuman(user))
		...
		if(H.a_intent == INTENT_HARM && H.potential >= 4)
			...
			playsound(get_turf(src), 'modular_darkpack/sound/bump.ogg', 100, FALSE)
			get_damage(10)
...
/obj/vampire_car
	...
	icon_state = "2"
	icon = 'modular_darkpack/icons/cars.dmi'
	anchored = TRUE
	plane = GAME_PLANE
	layer = CAR_LAYER
	density = TRUE
	...
	var/exploded = FALSE
	var/beep_sound = 'modular_darkpack/sound/beep.ogg'
...
/obj/vampire_car/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/gas_can))
		...
		if(G.stored_gasoline && gas < 1000 && isturf(user.loc))
			...
			playsound(loc, 'modular_darkpack/sound/gas_fill.ogg', 25, TRUE)
			...
			if(!repairing)
				...
				if(do_mob(user, src, 20 SECONDS))
					...
					if (roll <= 1)
						...
						playsound(src, 'modular_darkpack/sound/open.ogg', 50, TRUE)
					else
						to_chat(user, "<span class='warning'>You've failed to open [src]'s lock.</span>")
						playsound(src, 'modular_darkpack/sound/signal.ogg', 50, FALSE)
	...
	if(istype(I, /obj/item/melee/vampirearms/tire))
		if(!repairing)
			...
			if(do_mob(user, src, time_to_repair SECONDS))
				health = maxhealth
				playsound(src, 'modular_darkpack/sound/repair.ogg', 50, TRUE)
...
/datum/action/carr/baggage/Trigger()
	if(istype(owner.loc, /obj/vampire_car))
		...
		playsound(V, 'modular_darkpack/sound/door.ogg', 50, TRUE)
		if(STR.locked)
	...
	if(istype(owner.loc, /obj/vampire_car))
		...
		if(!V.on)
			if(V.health == V.maxhealth)
				V.on = TRUE
				playsound(V, 'modular_darkpack/sound/start.ogg', 50, TRUE)
			...
			V.on = FALSE
			playsound(V, 'modular_darkpack/sound/stop.ogg', 50, TRUE)
...
/obj/vampire_car/police
	...
	max_passengers = 3
	dir = WEST
	beep_sound = 'modular_darkpack/sound/migalka.ogg'
	access = "police"
	baggage_limit = 45
	baggage_max = WEIGHT_CLASS_BULKY
...
/obj/vampire_car/proc/handle_caring()
	...
	if(on)
		if(last_vzhzh+10 < world.time)
			playsound(src, 'modular_darkpack/sound/work.ogg', 25, FALSE)
	...
	if(adjusting_speed)
		if(on)
			if(adjusting_speed > 0 && speed_in_pixels <= 0)
				playsound(src, 'modular_darkpack/sound/stopping.ogg', 10, FALSE)
				...
				playsound(src, 'modular_darkpack/sound/drive.ogg', 10, FALSE)
		else
			if(adjusting_speed > 0 && speed_in_pixels < 0)
				playsound(src, 'modular_darkpack/sound/stopping.ogg', 10, FALSE)
