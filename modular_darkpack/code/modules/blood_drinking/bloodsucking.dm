/mob/living/carbon/human/proc/add_bite_animation()
	...
	var/mutable_appearance/bite_overlay = mutable_appearance('modular_darkpack/icons/icons.dmi', "bite", -BITE_LAYER)
...
/mob/living/carbon/human/proc/drinksomeblood(mob/living/mob)
	...
	qdel(suckbar)
	suckbar_loc = mob
	suckbar = image('modular_darkpack/icons/bloodcounter.dmi', suckbar_loc, "[round(14*(mob.bloodpool/mob.maxbloodpool))]", HUD_LAYER)
	suckbar.pixel_z = 40
	suckbar.plane = ABOVE_HUD_PLANE
	suckbar.appearance_flags = APPEARANCE_UI_IGNORE_ALPHA
	if(client)
		client.images += suckbar
	var/sound/heartbeat = sound('modular_darkpack/sound/drinkblood2.ogg', repeat = TRUE)
	...
	if(do_after(src, 30, target = mob, timed_action_flags = NONE, progress = FALSE))
		...
		if(HAS_TRAIT(src, TRAIT_FEEDING_RESTRICTION) && mob.bloodquality < BLOOD_QUALITY_NORMAL)	//Ventrue can suck on normal people, but not homeless people and animals. BLOOD_QUALITY_LOV - 1, BLOOD_QUALITY_NORMAL - 2, BLOOD_QUALITY_HIGH - 3. Blue blood gives +1 to suction
			...
			playsound(get_turf(src), 'modular_darkpack/sound/vomit.ogg', 75, TRUE)
			if(isturf(loc))
			...
			if(ishuman(mob) && !iskindred(mob))
				if(mob.stat != DEAD)
					if(isnpc(mob))
						...
						if(killed_count >= 5)
							SEND_SOUND(src, sound('modular_darkpack/sound/humanity_loss.ogg', 0, 0, 75))
							to_chat(src, "<span class='userdanger'><b>POLICE ASSAULT IN PROGRESS</b></span>")
					SEND_SOUND(src, sound('modular_darkpack/sound/feed_failed.ogg', 0, 0, 75))
