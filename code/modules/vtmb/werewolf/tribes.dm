...
/datum/action/gift/stoic_pose/Trigger()
	...
	if(allowed_to_proceed)
		playsound(get_turf(owner), 'sound/wod13/ice_blocking.ogg', 100, FALSE)
...
/datum/action/gift/freezing_wind/Trigger()
	. = ..()
	if(allowed_to_proceed)
		playsound(get_turf(owner), 'sound/wod13/wind_cast.ogg', 100, FALSE)
	...
	if(allowed_to_proceed)
		...
		if(C.pulling)
			if(isliving(C.pulling))
				...
				if(L.stat == DEAD)
					playsound(get_turf(owner), 'sound/wod13/bloody_feast.ogg', 50, FALSE)
					qdel(L)
...
/datum/action/gift/stinky_fur/Trigger()
	. = ..()
	if(allowed_to_proceed)
		playsound(get_turf(owner), 'sound/wod13/necromancy.ogg', 75, FALSE)
...
/datum/action/gift/venom_claws/Trigger()
	...
	if(allowed_to_proceed)
		if(ishuman(owner))
			playsound(get_turf(owner), 'sound/wod13/venom_claws.ogg', 75, FALSE)
...
/datum/action/gift/elemental_improvement/Trigger()
	...
	if(allowed_to_proceed)
		...
		if(ishuman(owner))
			playsound(get_turf(owner), 'sound/wod13/electro_cast.ogg', 75, FALSE)
			var/mob/living/carbon/human/H = owner
