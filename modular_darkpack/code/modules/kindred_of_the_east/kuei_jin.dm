...
/atom/breathing_overlay
	icon = 'modular_darkpack/icons/ui/kuei_jin.dmi'
	icon_state = "drain"
	alpha = 64
	density = FALSE
...
/atom/movable/screen/chi_pool
	name = "Chi Pool"
	icon = 'modular_darkpack/icons/ui/chi.dmi'
	icon_state = "base"
	layer = HUD_LAYER
	plane = HUD_PLANE
...
/atom/movable/screen/chi_pool/Initialize(mapload)
	. = ..()
	upper_layer = image(icon = 'modular_darkpack/icons/ui/chi.dmi', icon_state = "add", layer = HUD_LAYER+1)
							...
							if(H.CheckEyewitness(H, H, 7, FALSE))
								if(H.last_loot_check+50 <= world.time)
									...
									if(!H.warrant && !H.ignores_warrant)
										if(H.killed_count >= 5)
											H.warrant = TRUE
											SEND_SOUND(H, sound('modular_darkpack/sound/suspect.ogg', 0, 0, 75))
											to_chat(H, "<span class='userdanger'><b>POLICE ASSAULT IN PROGRESS</b></span>")
										else
											SEND_SOUND(H, sound('modular_darkpack/sound/sus.ogg', 0, 0, 75))
...
/datum/action/breathe_chi/proc/drain_breath(mob/living/carbon/human/kueijin, mob/living/victim)
	...
	chi_particle.icon = 'modular_darkpack/icons/ui/kuei_jin.dmi'
...
/datum/action/reanimate_yin/Trigger()
	...
	SEND_SOUND(usr, sound('modular_darkpack/sound/chi_use.ogg', 0, 0, 75))
