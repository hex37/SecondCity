...
/datum/action/blood_heal
	...
	button_icon = 'modular_darkpack/icons/ui/actions.dmi'
	background_icon_state = "discipline"
	icon_icon = 'modular_darkpack/icons/ui/actions.dmi'
	check_flags = AB_CHECK_HANDS_BLOCKED|AB_CHECK_IMMOBILE|AB_CHECK_LYING|AB_CHECK_CONSCIOUS
	vampiric = TRUE
	var/last_heal = 0
	var/level = 1
		...
		if(owner.client)
			if(owner.client.prefs)
				if(owner.client.prefs.old_discipline)
					button_icon = 'modular_darkpack/icons/disciplines.dmi'
					icon_icon = 'modular_darkpack/icons/disciplines.dmi'
				else
					button_icon = 'modular_darkpack/icons/ui/actions.dmi'
					icon_icon = 'modular_darkpack/icons/ui/actions.dmi'
	. = ..()
...
/datum/action/blood_heal/Trigger()
	if(istype(owner, /mob/living/carbon/human))
		...
		if(H.bloodpool < 1)
			...
			SEND_SOUND(H, sound('modular_darkpack/sound/need_blood.ogg', 0, 0, 75))
			return
		if((last_heal + 30) >= world.time)
			return
		last_heal = world.time
		H.bloodpool = max(0, H.bloodpool-1)
		SEND_SOUND(H, sound('modular_darkpack/sound/bloodhealing.ogg', 0, 0, 50))
